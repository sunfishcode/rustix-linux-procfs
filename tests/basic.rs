...
fn test_proc_self() {
    ...
    let fd = rustix_linux_procfs::proc_self_fd().unwrap();
    assert_ne!(fd.as_raw_fd(), 0);
}

#[test]
fn test_status_twice() {
    let fd = rustix_linux_procfs::proc_self_status().unwrap();
    drop(fd);
    let fd = rustix_linux_procfs::proc_self_status().unwrap();
    drop(fd);
}
...
fn parallel_self_proc_status() {
    ...
    fn self_proc_status() {
        rustix_linux_procfs::proc_self_status().expect("error getting proc/self/status pid");
    }
